...
class MainViewController: UIViewController {
    ...
    // 저항값 (0~100%) — UserDefaults에 자동 저장
    private static let resistanceKey = "resistancePercent"
    private var resistancePercent: Double = 50 {
        didSet {
            UserDefaults.standard.set(resistancePercent, forKey: Self.resistanceKey)
        }
    }
    ...
    private func setupFrictionSlider() {
        ...
        // 저장된 저항값 불러오기 (없으면 기본값 50)
        let savedResistance = UserDefaults.standard.object(forKey: Self.resistanceKey) as? Double ?? 50
        resistancePercent = savedResistance
        ...
        frictionSlider.value = Float(savedResistance)
