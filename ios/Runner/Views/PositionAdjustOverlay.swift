...
class PositionAdjustOverlay: UIView {
    ...
    private func setupControlPanel() {
        ...
        NSLayoutConstraint.activate([
            ...
            dpad.topAnchor.constraint(equalTo: hint.bottomAnchor, constant: 12),
            dpad.centerXAnchor.constraint(equalTo: controlPanel.centerXAnchor),
            dpad.widthAnchor.constraint(equalToConstant: 144),
            dpad.heightAnchor.constraint(equalToConstant: 96),
    ...
    private func buildDPad() -> UIView {
        ...
        // Row 1: ▲▲ ▲ ▲▲  (fast / fine / fast)
        let fineUp  = makeArrowButton("▲",  dx: 0, dy: -step)
        let fastUp  = makeArrowButton("⬆",  dx: 0, dy: -step * 5)
        let fineDown  = makeArrowButton("▼",  dx: 0, dy: step)
        let fastDown  = makeArrowButton("⬇",  dx: 0, dy: step * 5)
        let fineLeft  = makeArrowButton("◀",  dx: -step, dy: 0)
        let fastLeft  = makeArrowButton("⬅",  dx: -step * 5, dy: 0)
        let fineRight = makeArrowButton("▶",  dx: step, dy: 0)
        let fastRight = makeArrowButton("➡",  dx: step * 5, dy: 0)

        // Horizontal stack (← ⬅ ▶ ➡)
        let hStack = UIStackView(arrangedSubviews: [fastLeft, fineLeft, fineRight, fastRight])
        hStack.spacing = 6

        let vStack = UIStackView(arrangedSubviews: [
            UIStackView(arrangedSubviews: [fastUp, fineUp]).then { $0.spacing = 6 },
            hStack,
            UIStackView(arrangedSubviews: [fineDown, fastDown]).then { $0.spacing = 6 },
        ])
    ...
    private func makeArrowButton(_ title: String, dx: CGFloat, dy: CGFloat) -> UIButton {
...
private class CrosshairView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        ctx.move(to: CGPoint(x: cx - r1 - 10, y: cy)); ctx.addLine(to: CGPoint(x: cx - r2, y: cy))
        ctx.move(to: CGPoint(x: cx + r2, y: cy));       ctx.addLine(to: CGPoint(x: cx + r1 + 10, y: cy))
        ctx.move(to: CGPoint(x: cx, y: cy - r1 - 10)); ctx.addLine(to: CGPoint(x: cx, y: cy - r2))
        ctx.move(to: CGPoint(x: cx, y: cy + r2));       ctx.addLine(to: CGPoint(x: cx, y: cy + r1 + 10))
        ctx.strokePath()
...
private extension UIStackView {
    func then(_ block: (UIStackView) -> Void) -> UIStackView {
